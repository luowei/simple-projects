 ...
 * Date: 2010-06-01
 */
var LzAeroWin = LzAeroWin || {};

(function($) {
	LzAeroWin = function(config)
	{
		this.initialize = function()
		{
			if($("#" + __config.WindowId).length==0){
				__buildDOM(function() {
					$("#" + __config.WindowId).AeroWindow(__config);	
				});
			}else{
				if(__config.WindowIframeUrl!=null && __config.WindowIframeUrl!=undefined){
					$('#WindowSearchiframe_'+ __config.WindowId).attr('src', __config.WindowIframeUrl);
				}
				$("#" + __config.WindowId).AeroWindow(__config);
			}

		};
		this._getPosition = function(o)
		{
		...
		{
			if(__config.WindowFlyfrom!=null && __config.WindowFlyfrom!=undefined && __config.WindowStartTop == 0 && __config.WindowStartLeft ==0 ){
				if($("#" + __config.WindowFlyfrom).length>0){
					var position = this._getPosition($("#" + __config.WindowFlyfrom));
					__config.WindowStartTop = position.t ;
					__config.WindowStartLeft = position.l ;
				}
			}
			var dialogEl = $("<div />").attr({
				id: __config.WindowId
				//,title: __config.title
			}).css({
				display: "none"
				//	,
				//position: "absolute",
				//top : __config.WindowStartTop,
				//left : __config.WindowStartLeft
			});
			...
			if(__config.WindowIframeUrl!=null && __config.WindowIframeUrl!=undefined){
				var iframediv = '<iframe id="WindowSearchiframe_'+ __config.WindowId +'" src="'+ __config.WindowIframeUrl +'" width="100%" height="100%" style="border: 0px;" frameborder="0"></iframe><div class="iframeHelper"></div>'
				$(dialogEl).append(iframediv);
			}
...
(function($){
  $.fn.extend({ 
    ...
    AeroWindow: function(options) {
      ...
      if (($('body').data(WindowID)) == null || ($('body').data(WindowID)) == undefined) {
        var $WindowAllwaysRegistered = false;
        //Register this Window
        $('body').data( WindowID , options);
        var o =options;
      } else {
        //Window exists
        var $WindowAllwaysRegistered = true;
        var o =$('body').data(WindowID) ;
      }
      ...
      if ($WindowAllwaysRegistered == true) {
        ...
        $('body').data( 'AeroWindowMaxZIndex' , Window.css('z-index'));
        if(o.WindowStatus=="minimized"){
        	$(this).find(".WinBtnSet.winreg").click();
        	//RegularWindow2();
        }
        return;
      }
...
//        WindowCallback:null
 ...
 //     var options = $.extend(defaults, options);
      ...
      return this.each(function() {
        ...
        if(o.WindowUrl!=null && o.WindowUrl!=''){
        	var WindowContent = "<img src=/images/main/loading.gif border=0>";
        }else if(o.Windowcontent!=null && o.Windowcontent!=''){
        	var WindowContent = o.Windowcontent;
        }else{
        ...
        var WindowTitle		= $(this).find(".title");
        
        if(o.WindowStartTop != 0 || o.WindowStartLeft !=0 ){
        	if ( $(window).width() < o.WindowStartLeft +  o.WindowWidth  +32 ){
        		o.WindowStartLeft = $(window).width() - o.WindowWidth  - 32 ;
        	}
			Window.css(
				{
					top:o.WindowStartTop,
					left:o.WindowStartLeft
				}
			);
        }
        ...
        if(o.WindowUrl!=null && o.WindowUrl!=''){
			$(".table-mm-content",this).load(o.WindowUrl, function(responseText, textStatus, XMLHttpReques) {
			});
        }
		if (o.WindowCallback !== null && o.WindowCallback !== undefined)
		{
			$(this).ready(o.WindowCallback);
		}
        ...
        if(o.WindowPositionTop == 'center') {
          o.WindowPositionTop = ($(window).height()/2) - o.WindowHeight/2 - 27;
        }
        ...
        if (o.WindowMaximize) {
          $(this).dblclick(function(ev) {
			...
			var tc = tg.className;
			if(tc=="table-mm" || tc=="table-bm" || tc=="table-lm"  || tc=="table-rm" ){
	            switch (o.WindowStatus) {
	              case 'regular':
	                MaximizeWindow();
	                break;
        ...
        BTNClose.click(function () {
          ...
          jQuery.event.remove($("#"+WindowID));
          jQuery.removeData($("#"+WindowID));
          $("body").removeData(WindowID);
          $("#"+WindowID).empty().remove(); 
          return(false);
        });
        ...
        if (o.WindowResizable){
          Window.resizable({
            minHeight: o.WindowMinHeight+62,
