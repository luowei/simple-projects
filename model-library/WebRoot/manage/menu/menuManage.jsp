...
<TABLE height="100%" cellSpacing=0 cellPadding=0 width="100%">
  <TBODY>
    <TR>
      ...
      <TD 
    style="FONT-SIZE: 18px; BACKGROUND-IMAGE: url(${ctx }/manage/images/bg_left_tc.gif); COLOR: white; FONT-FAMILY: system">菜单管理</TD>
    ...
    vAlign="top">
     <form action="${ctx }/manage/menu/list.do" method="post">
<table>
	<tr>
		<td height="23">菜单名</td>
		<td><input type="text" name="key" id="menuName" value="${menuName }"/></td>
		<td>开始时间</td>
		...
		<a href="${ctx }/manage/menu/toAdd/0.do">增加</a></td>
	</tr>
	<tr>
	<td height="22" colspan="7">
		 ...
		 <c:forEach items="${filterList}" var="condition" varStatus="s">
			...
			${condition.displayName }<a href="${ctx }/manage/menu/list.do${condition.url}">删除</a>
		 </c:forEach>
	</td>
	</tr>
</table>
     ...
     <TABLE class=gridView id=ctl00_ContentPlaceHolder2_GridView1 
      ...
      border=0>
                <TR >                
                  <TH>序号</TH>
                  <TH class=gridViewHeader scope=col>菜单Id 
                  <a href="${ctx }/manage/menu/list.do${order }id:asc">升序</a>
                  <a href="${ctx }/manage/menu/list.do${order }id:desc">降序</a></TH>
                  <TH class=gridViewHeader scope=col>菜单名</TH>
                  <TH class=gridViewHeader scope=col>菜单排序
                  <a href="${ctx }/manage/menu/list.do${order }orders:asc">升</a>
                  <a href="${ctx }/manage/menu/list.do${order }orders:desc">降</a>
                  </TH>
                  <TH class=gridViewHeader scope=col>权限</TH>
                  <TH class=gridviewHeader scope=col>
             	     时间
                  <a href="${ctx }/manage/menu/list.do${order }createTime:asc">升</a>
                  <a href="${ctx }/manage/menu/list.do${order }createTime:desc">降</a>
				 </TH>
                ...
                <TR>
                  ...
                  <TD class=gridViewItem>${menu.id}</TD>
                  <TD class=gridViewItem>${menu.menuName}</TD>
                  <TD class=gridViewItem>${menu.orders}</TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="${ctx }/manage/menu/toAdd/${menu.id}.do">添加子菜单</A></TD>
                  <TD class=gridViewItem>${menu.createTime}</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="${ctx }/manage/menu/edit/${menu.id}.do">编辑</A></TD>
                  <TD class=gridViewItem><A class=cmdField 
            href="javascript:if(confirm('确实要删除吗?'))location.href='${ctx }/manage/menu/delete/${menu.id}.do';">删除</A> </TD>
                </TR>
             </c:forEach>
            </TABLE>
            ${tag }
