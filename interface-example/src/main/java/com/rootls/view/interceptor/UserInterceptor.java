package com.rootls.view.interceptor;
...
import static com.rootls.bean.Config.SESSION_MAX_INTERVAL;
import static com.rootls.bean.Config.loginCache;
import static com.rootls.utils.IPMacUtil.validBindIp;
import static com.rootls.utils.Md5Util.generatePassword;
...
public class UserInterceptor extends HandlerInterceptorAdapter {
    ...
    private boolean tooFrequent(HttpServletRequest request, HttpServletResponse response)
        ...
        Long lastLogin = Config.loginCache.get(username);
        if (lastLogin == null) {
            lastLogin = 0l;
            Config.loginCache.put(username, lastLogin);
        }
        ...
        Config.loginCache.put(username, currentTimeMillis());
        return false;
    }
    ...
    private boolean checkUserLogin(HttpServletRequest request) {
        ...
        if (okUser) {
            request.getSession().setMaxInactiveInterval(Config.SESSION_MAX_INTERVAL);
    ...
    private boolean checkInterfaceRight(HttpServletRequest request, User user, String clientIp) {
        ...
        if (request.getServletPath().startsWith("/trade")) {
            okUser = user == null ? false : checkIP(clientIp, user.getIpRight())
                    && user.getPriceFlag() != null && user.getPriceFlag().equals(1) && isNotBlank(user.getStartYearMonth());
